...
def shortestPath(G,start,end):
  ...
  Path.reverse()
  # Rob here: D is the value dict, I want final value
  return Path, D[Path[-1]]

#######################################################################
## END OF dijkstra and prioritydict
#######################################################################

#######################################################################
## Start interface with concorde
#######################################################################
...
def solve_tsp(adj, filename=path+'_tmp_'):
  ...
  prc.wait()
  with open('{0}.sol'.format(filename), 'r') as fp:
    ll = (''.join(fp)).split()
  result = [int(s) for s in ll[1:]]
  for s in ('{0}.dat'.format(filename), '{0}.sol'.format(filename)): 
    os.remove(s)
  return result
